 ...
 * getter/setter convention on a best attempt basis. The replication is typically recursive in that
 * the whole object graph of the input object is replicated into an equivalent output object graph,
 * resolving circular references, and eagerly fetching proxied instances as necessary. However, the
 * exact behavior of the replication process including
 * <ul>
 ...
 * <p>
 * <ol>
 * <li>All the configurable options of {@link BeanPopulatorBaseSpi} are available, as the
 * replication of JavaBean properties inevitably involves bean population.</li>
 * <p>
 * <li>If the default property filter {@link HibernatePropertyFilter} is used,
 * <ul>
 * <li>An application package prefix used to determine if a property with a type of an entity bean
 * class will be included for replication;</li>
 ...
 * </ul>
 * <p>
 * <li>For anything else that the existing implementation fails to transform, client can provide one
 * or multiple custom transformer factories via
 * {@link #initCustomTransformerFactory(net.sf.beanlib.spi.CustomBeanTransformerSpi.Factory...)}.</li>
 * </ol>
 * <p>
 * This was originally the base class for both the Hibernate 2.x and Hibernate 3.x replicators, but
 * now Hibernate 2 is no longer supported.
 *
...
public abstract class HibernateBeanReplicator implements BeanPopulatorBaseSpi {
  ...
  /**
   * Returns a copy of the given object. The exact behavior of the copy depends on how the
   * replicator has been configured via the init* methods.
   * <p>
   * In the case when none of the init* methods is invoked, this method behaves like
   * {@link HibernateBeanReplicator#deepCopy(Object)}, but without using the
   * {@link HibernatePropertyFilter}.
   *
  ...
  /**
   * Returns an instance of the given class with values copied from the given object. The exact
   * behavior of the copy depends on how the replicator has been configured via the init* methods.
   * <p>
   ...
   * Notes:
   * <ul>
   * <li>Use {@link HibernateBeanReplicator#copy(Object, Class)} instead of this method if you want
   * to plug in a different (detailed) property filter.</li>
   * </ul>
   *
  ...
  /**
   * Convenient method to deep copy the given object to an instance of the given class using the
   * default behavior.
   * <p>
   * Notes:
   * <ul>
  ...
  /**
   * Convenient method to shallow copy the given object using the default behavior. Shallow copy
   * means skipping those properties that are of type collection, map or under a package that
   * doesn't start with "java.".
   * <p>
   * Notes:
   * <ul>
   * <li>Use {@link HibernateBeanReplicator#copy(Object, Class)} instead of this method if you want
  ...
  /**
   * Convenient method to shallow copy the given object to an instance of the given class using the
   * default behavior. Shallow copy means skipping those properties that are of type collection, map
  ...
  public final <T> T shallowCopy(Object from, Class<T> toClass) {
    Set<? extends CollectionPropertyName<?>> emptyCollectionPropertyNameSet = Collections
        .emptySet();
    Set<Class<?>> emptyEntityBeanClassSet = Collections.emptySet();

    hibernateBeanTransformer.initPropertyFilter(new HibernatePropertyFilter()
        .withCollectionPropertyNameSet(emptyCollectionPropertyNameSet).withEntityBeanClassSet(
            emptyEntityBeanClassSet));
    return this.copy(from, toClass);
  }
  ...
  public final HibernateBeanReplicator initCustomTransformerFactory(
    ...
    if (customBeanTransformerFactories != null && customBeanTransformerFactories.length > 0) {
      hibernateBeanTransformer.initCustomTransformerFactory(
          customBeanTransformerFactories.length == 1 ? customBeanTransformerFactories[0]
  ...
  /**
   * Returns the property filter that is used to control what properties get propagated across and
   * what get skipped.
   */
  @Override
  ...
  /**
   * Note this method is only applicable if either the {@link #copy(Object)} or
   * {@link #copy(Object, Class)} is directly invoked, and is ignored otherwise (ie ignored if deep
   * or shallow copy is invoked instead).
   *
   * @param propertyFilter is similar to {@link DetailedPropertyFilter} but with a simpler API that
   *          is used to control whether a specific JavaBean property should be propagated from a
   *          source bean to a target bean.
   ...
   * or shallow copy is invoked instead).
   *
   * @param detailedPropertyFilter is used to control whether a specific JavaBean property should be
   *          propagated from the source bean to the target bean.
   * @return the current object (ie this) for method chaining purposes.
   */
  @Override
  public final HibernateBeanReplicator initDetailedPropertyFilter(
      DetailedPropertyFilter detailedPropertyFilter) {
    this.hibernateBeanTransformer.initDetailedPropertyFilter(detailedPropertyFilter);
    return this;
  }

  /**
   * Used to configure a call-back (to produce whatever side-effects deemed necessary) that is
   * invoked after the property value has been retrieved from the source bean, but before being
   * propagated across to the target bean.
   *
   * @param beanSourceHandler can be used to act as a call-back (to produce whatever side-effects
   *          deemed necessary) after the property value has been retrieved from the source bean,
   ...
   *
   * @param setterMethodCollector can be used to collect the property setter methods of a target
   *          JavaBean.
   * @return the current object (ie this) for method chaining purposes.
   */
  @Override
  ...
  public HibernateBeanReplicator initBeanPopulationExceptionHandler(
    ...
    this.hibernateBeanTransformer.initBeanPopulationExceptionHandler(
   ...
   *
   * @param baseConfig is used to conveniently group all the other initializable options into a
   *          single unit.
   * @return the current object (ie this) for method chaining purposes.
   */
  @Override
  ...
  /**
   * Convenient method to return the current property filter as HibernatePropertyFilter if the type
