 ...
 * <h2>Quick Start</h2> To replicate a simple JavaBean with a class definition like: <blockquote>
 *
 * <pre>
 ...
 * </pre>
 *
 * </blockquote> Notes a no-arg constructor is required for the JavaBean at the top level.
 * <p>
 * How about a more complex example ? Let's try:
 *
 * <pre>
 * <blockquote>public class ComplexBean {
 *     private String name;
 ...
 * </pre>
 *
 * First, set up the bean <blockquote>
 *
 * <pre>
 ...
 * </pre>
 *
 * </blockquote> And then replicate it in the same way:
 *
 * <pre>
 * <blockquote>ComplexBean to = new BeanReplicator().replicateBean(from);
 * <blockquote>
 * </pre>
 *
 * Voila! The "to" and "from" beans are different object instances, but the content (ie the entire object graph) has
 * been replicated. Note this works as long as these objects follow the JavaBean convention. See <a href=
 * "http://beanlib.svn.sourceforge.net/viewvc/beanlib/trunk/beanlib-test/src/net/sf/beanlib/provider/replicator/BeanReplicatorTest.java?revision=283&view=markup"
...
public class BeanReplicator extends ReplicatorTemplate implements BeanReplicatorSpi {
    ...
    /**
     * Convenient constructor that both defaults to use {@link BeanTransformer}, and allows plugging in one or more
     * custom bean transformer factories that will be chained together.
     *
    ...
    /**
     * Replicates the properties of a JavaBean object to an instance of a target class, which is selected from the given
     * "from" and "to" classes, giving priority to the one which is more specific whenever possible.
     *
     * @param <V> from type
     * @param <T> target type
    ...
    protected <V, T> T replicateBean(V from, Class<T> toClass, V originalFrom) {
        ...
        try {
            ...
            throw new BeanlibException(
                    "BeanReplicator.replicateBean failed:" + " toClass=" + toClass + ", from=" + from + ", originalFrom=" + originalFrom, e);
