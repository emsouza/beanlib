...
public enum ClassUtils {
    ...
    public static boolean immutable(Class<?> c) {
        if (c == null)
            return false;
        return c == String.class || c.isPrimitive() || (c.getModifiers() & ENUM) != 0 || Number.class.isAssignableFrom(c) && isJavaPackage(c)
                || Boolean.class == c || Character.class == c || Byte.class == c || Currency.class == c;
    }
    ...
    /**
     * Returns the fully qualified class name of the given class but without any array prefix such as "[L"; or null if
     * the given class is null; or an empty string if the given class is a primitive array which may be single or
